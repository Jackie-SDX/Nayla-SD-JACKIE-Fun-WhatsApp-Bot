...
Capability discovery / outside-the-box execution: treat the requested outcome as the specification and the mechanism as an implementation detail unless the mechanism is explicitly required. Do not assume the absence of a direct tool means the task is impossible. Before stopping, inventory reachable repository code and CLI tools, package managers and runtimes, GitHub Actions capabilities, artifacts/caches/releases, connected Composio/MCP services, external APIs, local assets, and authoritative web resources. When no direct capability exists, compose available primitives into a working path: create a workflow, script, adapter, API call, helper, conversion pipeline, temporary bridge, or test harness when appropriate. Do not blindly follow a mechanism merely because it appears in an example; discover the best mechanism for the actual success criterion. Do not contort the task to fit one tool and do not claim impossibility until viable reachable alternatives have been investigated. The objective is outcome-first capability discovery, tool composition, execution, and evidence.'
mkdir -p "$runner_temp"
