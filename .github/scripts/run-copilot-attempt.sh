...
attempt="$${1:-unknown}"
raw_log="$$(mktemp "$${RUNNER_TEMP:-/tmp}/copilot-$${attempt}-raw.XXXXXX")"
safe_log="$${RUNNER_TEMP:-/tmp}/copilot-$${attempt}-safe.log"
mcp_config=""

cleanup() { rm -f "$raw_log" "$mcp_config"; }
trap cleanup EXIT
...
if [[ -n "$${COMPOSIO_API_KEY:-}" ]]; then
  mcp_config="$$(mktemp "$${RUNNER_TEMP:-/tmp}/copilot-mcp.XXXXXX.json")"
