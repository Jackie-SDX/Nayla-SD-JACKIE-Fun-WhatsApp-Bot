...
if [[ -z "$run_id" ]]; then
  gh issue comment "$target" --body '## /oc retry failed jobs' --body 'No registered prior /oc run was found for this issue. Refusing to guess at an unrelated Actions run.'
  exit 0
fi
